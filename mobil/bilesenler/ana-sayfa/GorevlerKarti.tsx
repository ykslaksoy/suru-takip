...
import { getGorevOzeti, type Gorev, type GorevSeviye } from '@/kaynak/gorevler';
...
/** Ana sayfa özeti — tüm görevler /gorevler’de */
export function GorevlerKarti() {
  const scheme = useColorScheme() ?? 'light';
  const colors = Colors[scheme];
  const { ready, refreshKey } = useDatabase();
  const [maddeler, setMaddeler] = useState<Gorev[]>([]);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      setMaddeler(await getGorevOzeti(3));
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      <View style={styles.header}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.sub, { color: colors.textSecondary }]}>Planlananlar burada</Text>
        </View>
        <Pressable
