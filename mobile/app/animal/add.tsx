...
import { countAnimals, upsertAnimal } from '@/lib/database';
import { validateGehisId, validateTurkvetNo } from '@/lib/turkvet';
  ...
  const save = async () => {
    ...
    if (count >= limit) {
      Alert.alert('Limit aşıldı', `Mevcut paketiniz en fazla ${limit} hayvan destekler. Abonelik yükseltin.`, [
        { text: 'Tamam' },
        { text: 'Abonelik', onPress: () => router.push('/subscription') },
      ]);
      return;
    }
    if (!form.earTag.trim()) {
